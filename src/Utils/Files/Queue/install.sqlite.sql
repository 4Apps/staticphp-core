...
--
-- sqlite has no SELECT ... FOR UPDATE SKIP LOCKED, so reserving relies on the guarded
-- UPDATE alone. That is correct with any number of workers, but sqlite serialises writers,
-- so running more than one worker against it buys nothing. Treat this as the development
-- and test schema.
...
CREATE TABLE queue_jobs (
    ...
    -- Text timestamps in UTC, written by the application. 'YYYY-MM-DD HH:MM:SS' sorts and
    -- compares chronologically as a string, which is the whole reason for that format.
    --
