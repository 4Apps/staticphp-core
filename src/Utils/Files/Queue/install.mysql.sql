-- Job queue schema for mysql and mariadb.
--
...
--
-- Reserving uses SELECT ... FOR UPDATE SKIP LOCKED, which needs MySQL 8.0 or MariaDB 10.6.
-- Older servers still run the queue correctly - the claim is a guarded UPDATE either way -
-- but two workers will queue behind each other's row locks instead of stepping past them.
...
CREATE TABLE queue_jobs (
    ...
    -- written. 190 characters is what InnoDB can index under utf8mb4.
    ...
    -- Sized for identifiers and arguments; a job that needs a document should queue a
    -- reference to it rather than the document.
    ...
    -- way in and out, and every value here is written by the application in UTC.
    --
    ...
    KEY idx_queue_jobs_reserve (queue, priority DESC, available_at, id),

    -- Multiple NULLs are permitted in a unique index, which is what makes one index cover
    -- both "at most one pending job with this key" and "most jobs have no key".
    UNIQUE KEY idx_queue_jobs_unique_key (unique_key)
...
CREATE TABLE queue_failed_jobs (
    ...
    PRIMARY KEY (id),
    KEY idx_queue_failed_jobs_failed_at (failed_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
